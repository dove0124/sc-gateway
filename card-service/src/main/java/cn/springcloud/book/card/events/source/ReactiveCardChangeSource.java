package cn.springcloud.book.card.events.source;
...
import cn.springcloud.book.card.event.models.CardChangedEvent;
...
public class ReactiveCardChangeSource {
    ...
    public ReactiveCardChangeSource() {
        this.flux = Flux.<Message<CardChangedEvent>>create(sink -> this.eventSink = sink).publish()
                                                                                         .autoConnect();
    }

    private Mono<Void> publishReactiveCardChange(String operation, Card card) {
        logger.info("Sending message for Card Id: {}", card.getId());
        ...
        return monoEvent.map(event -> eventSink.next(MessageBuilder.withPayload(event)
                                                                   .build()))
                        .then();
    }
